...
if (empty($household_id)) {
  $sql = "SELECT  DISTINCT member_id FROM household_members WHERE is_military = 1";
  $stmt = $conn->prepare($sql);
}
else{ 
  $sql = "SELECT * FROM household_members WHERE household_id = ? AND is_military = 1";
  $stmt = $conn->prepare($sql);
...
if ($result->num_rows == 0) {
  $_SESSION['error'] = "No Military found in this household";
...
if (empty($household_id)) {
  $filename = 'MILITARY_LIST_' . date('Y-m-d') . '.docx';
}
else{
  $filename = 'MILITARY_LIST_' . date('Y-m-d') . ' Blk' . $household_row['Blk'] . " Lot" . $household_row['Lot']. '.docx';
}
