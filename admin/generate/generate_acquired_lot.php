...
if (empty($household_id)) {
  $sql = "SELECT * FROM members WHERE lot_type = 'Acquired'";
...
if ($result->num_rows == 0) {
  $_SESSION['error'] = "No Acquired Lots found";
...
if (empty($household_id)) {
  $filename = 'ACQUIRED_LOTS_' . date('Y-m-d') . '.docx';
}
else{
  $filename = 'ACQUIRED_LOTS_' . date('Y-m-d') . ' Blk' . $household_row['Blk'] . " Lot" . $household_row['Lot']. '.docx';
}
