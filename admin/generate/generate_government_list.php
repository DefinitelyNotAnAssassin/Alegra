...
if (empty($household_id)) {
  $sql = "SELECT  DISTINCT member_id FROM household_members WHERE is_sss = 1 OR is_gsis = 1";
  $stmt = $conn->prepare($sql);
}
else{ 
  $sql = "SELECT * FROM household_members WHERE household_id = ? AND (is_sss = 1 OR is_gsis = 1)";
  $stmt = $conn->prepare($sql);
...
if ($result->num_rows == 0) {
  $_SESSION['error'] = "No SSS/GSIS found in this household";
...
if (empty($household_id)) {
  $filename = 'SSS_GSIS_LIST_' . date('Y-m-d') . '.docx';
}
else{
  $filename = 'SSS_GSIS_LIST_' . date('Y-m-d') . ' Blk' . $household_row['Blk'] . " Lot" . $household_row['Lot']. '.docx';
}
