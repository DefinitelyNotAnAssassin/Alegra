...
if (empty($household_id)) {
  $sql = "SELECT * FROM members WHERE award_type = 'Awarded'";
...
if (empty($household_id)) {
  $filename = 'AWARDED_LOTS_' . date('Y-m-d') . '.docx';
}
else{
  $filename = 'AWARDED_LOTS_' . date('Y-m-d') . ' Blk' . $household_row['Blk'] . " Lot" . $household_row['Lot']. '.docx';
}
